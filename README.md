...
```js
var sql = jSql.select('user_table, member_table', {
...
```js
var sql = jSql.select('user_table u, member_table m', {
  'u.name': 'name',
  'u.age': 'age',
  'u.point': 'point',
...
### sql update
```js
var sql = jSql.update('user_table').set({
  name: 'hao',
  age: 22,
  point: 1200,
...
### sql insert
```js
var sql = jSql.insert('user_table').set({
  name: 'hao',
  age: 12,
  point: 3333,
...
### sql delete
```js
var sql = jSql.del('user_table').where({
      'name': ['=', 'hao'],
      'age': ['<>', 12, 23],
      'point': ['>', 1200]
    }).done();
